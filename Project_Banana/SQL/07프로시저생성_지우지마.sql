...
-- 광고 수락 프로시저
CREATE OR REPLACE PROCEDURE PRC_ADV_SUCCESS
(
 V_ADV_APPLY_CODE          IN ADV_APPLY.ADV_APPLY_CODE%TYPE -- 수락할 광고 신청 코드
 , V_ADMIN_CODE                IN ADMIN.ADMIN_CODE%TYPE -- 수락할 관리자 코드
 , V_URL1                 IN  ALARM.URL%TYPE -- 취소 알람 URL
, V_URL2                 IN  ALARM.URL%TYPE -- 성사 알람 URL
)
IS

V_REJECT_ADV_APPLY_CODE     ADV_APPLY.ADV_APPLY_CODE%TYPE; -- 취소당할 광고신청코드
V_B_USER_CODE                           B_USER.B_USER_CODE%TYPE;     -- 수락된사람코드
V_REJECT_B_USER_CODE                           B_USER.B_USER_CODE%TYPE;     -- 취소된사람코드 

-- 커서 선언
CURSOR CUR_DELETE_ADV
IS
SELECT ADV_APPLY_CODE, B_USER_CODE FROM ADV_APPLY
WHERE ADV_APPLY_CODE <>(SELECT ADV_APPLY_CODE FROM ADV_APPLY_ACCEPT)
AND START_DATE <= (SELECT END_DATE FROM ADV_APPLY WHERE ADV_APPLY_CODE=V_ADV_APPLY_CODE) 
AND END_DATE >=(SELECT START_DATE FROM ADV_APPLY WHERE ADV_APPLY_CODE=V_ADV_APPLY_CODE)
AND ADV_APPLY_CODE !=V_ADV_APPLY_CODE
AND ADV_TYPE_CODE =(SELECT ADV_TYPE_CODE FROM ADV_APPLY WHERE ADV_APPLY_CODE=V_ADV_APPLY_CODE)
;
...
FROM ADV_APPLY
WHERE ADV_APPLY_CODE = V_ADV_APPLY_CODE;
...
LOOP
FETCH CUR_DELETE_ADV INTO V_REJECT_ADV_APPLY_CODE, V_REJECT_B_USER_CODE;
EXIT WHEN CUR_DELETE_ADV%NOTFOUND;
-- 중복되는신청 상게 
DELETE FROM ADV_APPLY WHERE ADV_APPLY_CODE=V_REJECT_ADV_APPLY_CODE;


-- 취소된사람들에게 알람 
    PRC_ALARM('AR_C31',V_URL1,V_REJECT_B_USER_CODE);   -- 광고신청이 취소 취소 되었습니다.



END LOOP;

CLOSE CUR_DELETE_ADV;


-- 광고 신청 수락 테이블에 해당 광고 적재
INSERT INTO ADV_APPLY_ACCEPT(ADV_APPLY_ACCEPT_CODE, ADV_APPLY_CODE, ADMIN_CODE)
    VALUES('ADV_AC'||SEQ_ADV_ACC.NEXTVAL, V_ADV_APPLY_CODE, V_ADMIN_CODE);

-- 신청완료 알람 
    PRC_ALARM('AR_C12', V_URL2, V_B_USER_CODE);


-- COMMIT;
END;

EXEC PRC_ADV_SUCCESS('ADV_AP6','ADMIN4',' ',' ');